...
function CurrentVideo(params) {
    ...
    const [rate_up, setRateUp] = useState(0);
    const [rate_down, setRateDown] = useState(0);
    ...
    function ratingUp(e, id) {
        request("post", 'http://localhost:8080/api/rating/' + id + "/up")
            .then(function (response) {
                //todo сделать ограничение по рейтингу
                setRateUp(response.data.rating_up)
            })
            .catch(function (error) {
                console.error("err rate_up")
            })
            .finally(function () {
                // выполняется всегда
            });
    ...
    function ratingDown(e, id) {
        request("post", 'http://localhost:8080/api/rating/' + id + "/down")
            .then(function (response) {
                //todo сделать ограничение по рейтингу
                setRateDown(response.data.rating_down)
            })
            .catch(function (error) {
                console.error("err rate_down")
            })
            .finally(function () {
                // выполняется всегда
            });
    ...
    function getCurrentVideo() {
        request("get", 'http://localhost:8080/api/video/' + window.location.pathname.substring(window.location.pathname.lastIndexOf("/") + 1))
            .then(function (response) {
                ...
                setRateUp(response.data.rating.rating_up)
                setRateDown(response.data.rating.rating_down)
            })
            .catch(function (error) {
                setCurrVid({
    ...
    function getSuggests() {
        request("get", 'http://localhost:8080/api/videos')
            .then(function (response) {
                let videos = []
                        ...
                        <div className="row p-2">
                            ...
                            <div className="col m-0 p-0 text-center">
                                <div className="btn-group" role="group" aria-label="Basic mixed styles example">
                                    <button onClick={(e) => ratingUp(e, currVid.rating_id)} className="btn bg-success btn-lg">
                                    ...
                                    <button onClick={(e) => ratingDown(e, currVid.rating_id)} className="btn bg-danger btn-lg">
                                        <span className="me-1">{rate_down}</span>
